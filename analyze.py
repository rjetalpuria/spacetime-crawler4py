...
hash_val = {}  # to store the urls are its respective hash values
...
# prints the top n words and frequencies in a sorted descendingly based on values and breaking ties
# with alphabetical order
def printTopNFreq(frequencies, n):
    frequencies = dict(sorted(frequencies.items())) # sort by alphabetical order first
    # sort by frequency in descending order
    frequencies = dict(sorted(frequencies.items(), key=lambda tup: tup[1], reverse=True))
    # print
